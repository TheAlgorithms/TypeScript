 ...
 * @return {number} - index of the target number in the list, or actual position of the element to be placed if not found. 
 * @see [BinarySearch](https://www.geeksforgeeks.org/binary-search/)
 * @example binarySearch([1,2,3], 2) => 1
 * @example binarySearch([4,5,6], 2) => 0
 */
...
export const binarySearchRecursive = (array: number[], target: number, start = 0, end = array.length - 1): number => {
    ...
    // if the target is greater than the middle value, move the start pointer to be middle + 1 to narrow the search space
    // otherwise, move the end pointer to be middle - 1 
    if (array[mid] < target) {
